    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-lg border bg-card p-4">
          ...
          <p className="text-2xl font-bold">{data.phase_breakdown.opening.toFixed(1)} ACPL</p>
        </div>
        <div className="rounded-lg border bg-card p-4">
          ...
          <p className="text-2xl font-bold">{data.phase_breakdown.middlegame.toFixed(1)} ACPL</p>
        </div>
        <div className="rounded-lg border bg-card p-4">
          ...
          <p className="text-2xl font-bold">{data.phase_breakdown.endgame.toFixed(1)} ACPL</p>
        </div>
      </div>
      ...
      <div className="rounded-lg border bg-card p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-3xl font-bold">{(data.rushing_analysis.normal_blunder_rate * 100).toFixed(1)}%</p>
            <p className="text-sm text-muted-foreground">Normal blunder rate</p>
          </div>
          <div>
            <p className="text-3xl font-bold text-eval-blunder">{(data.rushing_analysis.time_pressure_blunder_rate * 100).toFixed(1)}%</p>
            <p className="text-sm text-muted-foreground">Under {data.rushing_analysis.time_pressure_threshold}s blunder rate</p>
          </div>
          <div>
            <p className="text-3xl font-bold">{data.rushing_analysis.rushing_multiplier.toFixed(1)}x</p>
      ...
      <div className="rounded-lg border bg-card p-6">
        ...
        <div className="flex items-end gap-1 h-32">
          {data.blunder_heatmap.map((b) => {
            const maxCount = Math.max(...data.blunder_heatmap.map((x) => x.count), 1);
